...
open class JiraClient(private val setup: IssueTrackingApplication) :
  ...
  override fun getProprietaryIssue(issue: Issue): JiraIssue? {
    ...
    return queryIssue(keyFieldMapping) ?: keyFieldMapping.getCallback()
      ?.let { return queryIssue(it) }
  }
  ...
  override fun getLastUpdated(internalIssue: JiraIssue): LocalDateTime =
    LocalDateTime.ofInstant(
      Instant.ofEpochMilli(internalIssue.updateDate.millis),
  ...
  private fun convertFromMetadataId(fieldName: String, value: Any): Any {
    return when {
      "priorityId" == fieldName -> JiraMetadata.getPriorityName(
  ...
  private fun createTargetIssue(
    ...
    val targetIssue =
      getProprietaryIssue(basicIssue.key)
        ?: throw IssueClientException("Failed to locate newly created issue")
  ...
  override fun changedIssuesSince(
    ...
    var jql =
      "(updated >= '$lastPollingTimestampAsString' OR created >= '$lastPollingTimestampAsString')"
  ...
  override fun getAttachments(internalIssue: JiraIssue): List<Attachment> =
    internalIssue.attachments?.map { jiraAttachment ->
      val attachmentInputStreamPromise =
        jiraRestClient.issueClient.getAttachment(jiraAttachment.contentUri)
  ...
  override fun getStateHistory(internalIssue: JiraIssue): List<StateHistory> {
    val result =
      jiraRestClient.issueClient.getIssue(
        internalIssue.key,
        listOf(IssueRestClient.Expandos.CHANGELOG)
      ).claim()
        .changelog
        ?.flatMap { logEntry ->
          logEntry.items
            ...
            .map {
              StateHistory(
                toLocalDateTime(logEntry.created),
                it.fromString ?: "",
                it.toString ?: ""
              )
            }
    ...
    val indexOfLast =
      maxOf(result.indexOfLast { it.fromState == "Open" || it.fromState == "Neu" }, 0)
  ...
  override fun setState(
    ...
    val transition =
      jiraRestClient.getHtmlRenderingRestClient().getAvailableTransitions(internalIssue.key)
        ...
        .keys.firstOrNull()
        ?: throw IllegalArgumentException("No transition found to state $targetState")
    try {
      jiraRestClient.issueClient.transition(internalIssue, TransitionInput(transition.id)).claim()
    } catch (e: Exception) {
      throw IllegalArgumentException(
        "Transition failed for issue ${internalIssue.key} to $targetState",
        e
      )
    }
  }
    ...
    jiraRestClient.getHtmlRenderingRestClient().getAvailableTransitions(internalIssue.key)
      ...
      .keys.firstOrNull()?.let {
        try {
          ...
          jiraRestClient.issueClient.transition(
            internalIssue,
            TransitionInput(it.id, listOf(resolution))
          )
            .claim()
  ...
  private fun getCustomFields(
    ...
    return field.value?.let { getArrayForJsonArrayValue(it) }?.takeIf { it.isNotEmpty() }
      ?: field.value
  }
  ...
  private fun toLocalDateTime(jodaDateTime: DateTime): LocalDateTime =
    LocalDateTime.ofInstant(
      Instant.ofEpochMilli(jodaDateTime.toInstant().millis),
      ZoneId.systemDefault()
    )
  ...
  private fun getRestExceptionMessage(exception: java.lang.Exception): String? {
    return when {
      ...
      exception.cause != null && exception.cause is RestClientException -> {
        val additional =
          (exception.cause as RestClientException).errorCollections.joinToString("\n") {
            it.errorMessages.joinToString("\n") { it2 -> it2.toString() } + "\n" +
