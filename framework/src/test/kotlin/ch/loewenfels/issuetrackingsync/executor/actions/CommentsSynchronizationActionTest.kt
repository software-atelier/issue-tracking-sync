...
internal class CommentsSynchronizationActionTest : AbstractSpringTest() {
  ...
  fun execute_someCommentsHaveContentFilterFiltersEverything_NoSync() {
    ...
    val targetClient =
      buildIssueTrackingClient(buildIssueTrackingApplication("RtcClient"), clientFactory)
  ...
  fun execute_targetContainsSyncedComment_nothingSynced() {
    ...
    `when`(targetClient.getComments(targetIssue)).thenReturn(
      listOf(
        getCommentSynced(
          "1234",
          "9999"
...
class CommentFilterAlwaysFalseStub(val filterProperties: Map<String, String> = emptyMap()) :
  CommentFilter {
  override fun getFilter(): (Comment) -> Boolean = { (_) -> false }

}

class CommentFilterAlwaysTrueStub(val filterProperties: Map<String, String> = emptyMap()) :
  CommentFilter {
