...
class FieldSkippingEvaluatorFactoryTest {
  ...
  fun getEvaluator_validClass_classLoaded() {
    // arrange
    val mapping =
      buildFieldMappingDefinition(mutableListOf(SkipUnlessAllowedState::class.java.name))
    // act
  ...
  fun getEvaluator_invalidClassName_exception() {
    // arrange
    val unknownMapping =
      buildFieldMappingDefinition(mutableListOf("ch.loewenfels.MapperDoesntExist"))
    // act
