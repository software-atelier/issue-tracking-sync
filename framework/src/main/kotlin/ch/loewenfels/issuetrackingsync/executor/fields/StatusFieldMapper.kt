 ...
 */
open class StatusFieldMapper(fieldMappingDefinition: FieldMappingDefinition) : FieldMapper,
  Logging {
  ...
  override fun <T> getValue(
    ...
    Pair(
      issueTrackingClient.getState(proprietaryIssue),
      issueTrackingClient.getStateHistory(proprietaryIssue)
    )
  ...
  override fun <T> setValue(
    ...
    if (states.isNotEmpty()) {
      ...
      val correspondingStates =
        (associations[finalState.toState].orEmpty()).split("[,;/]".toRegex())
  ...
  private fun getStatePath(
    sourceStateHistory: List<StateHistory>,
    ...
    sourceStateHistory.forEach { stateHistory ->
      ...
      if (correspondingStateInTargetWorld.contains(currentTargetState)) {
        ...
        val positionInMultiTransition =
          correspondingStateInTargetWorld.indexOf(currentTargetState) + 1
