...
class SynchronizationFlow(
  ...
  init {
    ...
    syncActions =
      syncFlowDefinition.actions.associateBy({ it }, { buildSyncAction(it, actionDefinitions) })
  ...
  private fun buildSyncPreAction(preActionDefinition: PreActionDefinition): PreAction {
    ...
    return try {
      preActionClass.getConstructor(PreActionDefinition::class.java)
        .newInstance(preActionDefinition)
  ...
  private fun buildSyncAction(
    ...
    return try {
      ...
      (actionClass.constructors.find { c -> c.parameterCount == 1 }?.newInstance(actionName)
        ?: actionClass.constructors.first { c -> c.parameterCount == 0 }
          .newInstance()) as SynchronizationAction
    } catch (e: Exception) {
      throw IllegalArgumentException(
  ...
  private fun loadInternalSourceIssue(issue: Issue, checkLastUpdatedTimestamp: Boolean = true) {
    val sourceIssue =
      sourceClient.getProprietaryIssue(issue.key)
        ?: throw IllegalArgumentException("No source issue found")
    if (checkLastUpdatedTimestamp) {
      val gap =
        issue.lastUpdated.until(sourceClient.getLastUpdated(sourceIssue), ChronoUnit.SECONDS)
    ...
    val keyFieldMapping =
      FieldMappingFactory.getKeyMapping(syncFlowDefinition.keyFieldMappingDefinition)
    keyFieldMapping.loadSourceValueWithCallback(issue, sourceClient)
    issue.keyFieldMapping = keyFieldMapping
  }

  private fun execute(
    syncActionEntry: Map.Entry<SyncActionName, SynchronizationAction>,
    issue: Issue
  ) {
    val actionDefinition =
      actionDefinitions.first { it.name.equals(syncActionEntry.key, ignoreCase = true) }
    val fieldMappings =
      actionDefinition.fieldMappingDefinitions.map { FieldMappingFactory.getMapping(it) }.toList()
